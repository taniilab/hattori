...
class Main(QtGui.QWidget):
    ...
    def __init__(self, parent=None):
        ...
        layout8.addWidget(def_wLabel)
        layout8.addWidget(self.def_wEdit)
        layout9 = QtGui.QHBoxLayout()
        layout9.addWidget(sigmoid_gainLabel)
        layout9.addWidget(self.sigmoid_gainEdit)
        layout10 = QtGui.QHBoxLayout()        
        layout10.addWidget(ab_refacttimeLabel)
        layout10.addWidget(self.ab_refacttimeEdit)
        layout11 = QtGui.QHBoxLayout()
        layout11.addWidget(ab_weakLabel)
        layout11.addWidget(self.ab_weakEdit)
        layout12 = QtGui.QHBoxLayout()
        layout12.addWidget(simtimeLabel)
        layout12.addWidget(self.simtimeEdit)
        layout13 = QtGui.QVBoxLayout()
        layout13.addWidget(startBtn)
        layout13.addWidget(saveBtn)
        ...
        vrm:静止膜電位[V]
        tau:積分発火モデルの時定数
        deltatime:1プロット分の時間[s]
        phigh:発火時電圧[V]
        def_w:シナプス結合荷重の初期値
        sigmoid_gain:発火電圧の揺らぎに用いるシグモイド関数のゲイン
        ab_refactftime:不応期時間[s]
        ab_weak:不応期の際の内部電圧の上がりにくさ(0(上がらない)　～　1(上がりやすい))
        sim time:シミュレーション時間    
        """
        self.neuron1 = Neuron3(int(self.numneuEdit.text()),
...
class TabwaveWidget(QtGui.QWidget):
    ...
    def plot(self, neuron):
        self.fig, self.ax = plt.subplots(nrows = neuron.numneu, figsize=(16, 10))
        ...
        for i in range(0, neuron.numneu):
            if neuron.numneu ==1:
                self.ax.set_title("Neuron" + str((i+1)))
            else:        
                self.ax[i].set_title("Neuron" + str((i+1)))
        ...
        for i in range(0, neuron.numneu):
            if neuron.numneu == 1:
                ...
                self.ax[i].set_ylabel("membrane potential [mV]")
                self.ax[i].set_xlabel("time [t]")
                self.ax[i].set_xlim((self.t.min(), self.t.max()))
                self.ax[i].set_ylim(-90, 50)
        ...
        self.fig.subplots_adjust(left=0.00, bottom=0.00, right=1.00, top=1.00, wspace=0.00, hspace=0.00)
        self.fig.tight_layout(pad=0.05, w_pad=0.00, h_pad=0.00)
        ...
        for i in range(0, np.size(self.t)):
            #neuron.input_whitenoise()        
            #neuron.input_bumpnoise()
            neuron.input[0][0] = 40
            vin, output = neuron.propagation("sigmoid")
            for j in range(0, neuron.numneu):        
                self.vplt[j][i] = vin[j]   
        ...
        for i in range(0, neuron.numneu):        
            ...
            if neuron.numneu == 1:
                self.lines.set_data(self.t, self.vplt[i])
                self.ax.fill_between(self.t, self.vplt[i], base, facecolor="thistle", alpha=0.2)           
            else:
        ...
        for i in range(0, neuron.numneu):
            for j in range(0, np.size(self.t)):
                if neuron.raster[i][j] != 0:
                    self.ax.vlines(self.t[j], neuron.raster[i][j] - 0.4, neuron.raster[i][j] + 0.4, 'k')
