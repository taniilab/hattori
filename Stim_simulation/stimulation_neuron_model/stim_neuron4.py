...
class HH_compartment:
    def __init__(self, sim_time_ms, dt_ms):
        ...
        self.I_membrane_total = np.zeros(allsteps)

        self.V_intra = -74 * np.ones(allsteps)
        self.V_extra = np.zeros(allsteps)


class Neuron:
    def __init__(self, sim_time_ms=180000, dt_ms=0.04, R_cell=10):
    ...
    def calc_current(self, cell_compartment, Vth=-56.2, tau_max=608, gleak=0.0205, eleak=-70.3,
        ...
        cell_compartment.I_membrane_total[self.curstep] = cell_compartment.Ileak[self.curstep] + cell_compartment.INa[
            self.curstep] + cell_compartment.IK[self.curstep] + cell_compartment.Im[self.curstep]

    def calc_potential(self, cell_left, cell_right, Cm=1):
        self.calc_current(cell_left)
        if self.curstep * self.dt_ms > 500:
            cell_left.intracellular_current[self.curstep] = - (
                        cell_left.V_intra[self.curstep] - cell_right.V_intra[self.curstep]) / self.R_cell
        dV_intra_left = (self.dt_ms * (
                    cell_left.I_membrane_total[self.curstep] + cell_left.intracellular_current[self.curstep]) / Cm)
        ...
        if self.curstep * self.dt_ms > 500:
            cell_right.intracellular_current[self.curstep] = - (
                    cell_right.V_intra[self.curstep] - cell_left.V_intra[self.curstep]) / self.R_cell

        dV_intra_right = (self.dt_ms * (
                cell_right.I_membrane_total[self.curstep] + cell_right.intracellular_current[self.curstep]) / Cm)
...
class PotentialStimulation:
    ...
    def FG_set_potential(self, stim_timing_ms, potential_mv):
        V_stim = np.zeros(self.allsteps)
        stim_timing_step = np.zeros(len(stim_timing_ms))

        if len(stim_timing_ms) == 1:
            V_stim[int(stim_timing_ms[0] / self.dt_ms):] = potential_mv[0]
            return [int(stim_timing_ms[0] / self.dt_ms)], V_stim
        else:
            ...
            for stim_i in range(0, len(stim_timing_step)-1):
                V_stim[int(stim_timing_step[stim_i]):int(stim_timing_step[stim_i + 1])] = potential_mv[stim_i]
            V_stim[int(stim_timing_step[len(stim_timing_step) - 1]):] = potential_mv[len(stim_timing_step) - 1]

            return stim_timing_step, V_stim


class ExperimentalSystem:
    def __init__(self, sim_time_ms, dt_ms,
                 electrode_potential_left, stimulation_timing_ms,
                 solution_R_left, solution_R_right):
        stimulation = PotentialStimulation(sim_time_ms=sim_time_ms, dt_ms=dt_ms)
        self.stim_timing_step, self.elecV_left = stimulation.FG_set_potential(stim_timing_ms=stimulation_timing_ms,
                                                                              potential_mv=electrode_potential_left)
        self.elecV_right = np.zeros(int(sim_time_ms / dt_ms))
        self.solR_left = solution_R_left
        self.solR_right = solution_R_right
        self.stim_timing_ms = stimulation_timing_ms
        self.stim_flag = False

        self.neuron = Neuron(sim_time_ms=sim_time_ms, dt_ms=dt_ms)

    def experiment(self):
        for step_i in range(0, self.neuron.allsteps - 1):
            if self.neuron.curstep in list(self.stim_timing_step):
                self.neuron.cell_left.V_extra[self.neuron.curstep] = self.elecV_left[self.neuron.curstep] * (
                        (self.neuron.R_cell + self.solR_right) / (
                        self.neuron.R_cell + self.solR_left + self.solR_right))
                self.neuron.cell_right.V_extra[self.neuron.curstep] = self.elecV_left[
                                                                          self.neuron.curstep] * (
                                                                              self.solR_right / (
                                                                              self.neuron.R_cell + self.solR_left + self.solR_right))
            elif self.stim_flag:
                self.neuron.cell_left.V_extra[self.neuron.curstep] = self.elecV_left[self.neuron.curstep] - self.neuron.cell_left.I_membrane_total[]

           self.neuron.calc_potential(self.neuron.cell_left, self.neuron.cell_right)



def main():
    save_path = 'C:/Users/ishida/Desktop/stim_neuron_sim/20200114'
    if not os.path.isdir(save_path):
        os.mkdir(save_path)
    if not os.path.isdir(save_path + '/picture'):
        os.mkdir(save_path + '/picture')

    left_stim_timing_ms = [1000, 1020, 1040]
    ...
    df_left['I_membrane_total [pA]'] = neuron.cell_left.I_membrane_total
    ...
    df_right['I_membrane_total [pA]'] = neuron.cell_right.I_membrane_total
    df_left.to_csv(save_path + '/cell_left_{0}Mohm_{1}_{2}.csv'.format(R,
                                                                       left_stim_ext_potential[0],
                                                                       left_stim_ext_potential[1]))
    df_right.to_csv(save_path + '/cell_right_{0}Mohm_{1}_{2}.csv'.format(R,
                                                                         left_stim_ext_potential[0],
                                                                         left_stim_ext_potential[1]))
    ...
    ax1.plot(neuron.time_seq / 1000, neuron.cell_left.V_intra, label='cell_left',
             color='darkcyan')
    ax1.plot(neuron.time_seq / 1000, neuron.cell_right.V_intra, label='cell_right',
             color='darkgreen')
    ax1.plot(neuron.time_seq / 1000, neuron.cell_left.V_extra, label='V_extra',
             color='darkmagenta')
    ...
    plt.tight_layout()
    plt.savefig(save_path + '/picture/cell_{0}Mohm_{1}_{2}.png'.format(R,
                                                                                          left_stim_ext_potential[0],
                                                                                          left_stim_ext_potential[1]),
        dpi=600)
    plt.show()
